...
;END AUTOCAST
;BEGIN CODE
;Debug.Trace("_SLS_: Piece of shit quest is starting")
_SLS_GuardWarnTypeAndCooldown.SetValueInt(4)
_SLS_GuardWarnDoApproach.SetValueInt(0)
kmyquest.SetNextEventGlobals()	;lives in parent script WorldInteractionsScript
Actor Guard = Alias_Guard.GetReference() as Actor ; Guard is not flagged as optional - don't need to worry if it's filled
If Guard.IsInFaction(_SLS_GuardBehaviourWarningFact)
	(Game.GetFormFromFile(0x000D64, "SL Survival.esp") as SLS_Utility).SendBeginGuardWarnPunishEvent(QuestToStop = Self, Guard = Guard)
EndIf
