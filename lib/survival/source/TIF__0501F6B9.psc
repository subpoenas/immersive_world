;BEGIN FRAGMENT CODE - Do not edit anything between this and the end comment
;NEXT FRAGMENT INDEX 1
Scriptname TIF__0501F6B9 Extends TopicInfo Hidden

;BEGIN FRAGMENT Fragment_0
Function Fragment_0(ObjectReference akSpeakerRef)
Actor akSpeaker = akSpeakerRef as Actor
;BEGIN CODE
ObjectReference NewTarget = Dnpc.GetReference()
_SLS_BeggingSelfDegradation.FailAllObjectives()
_SLS_BeggingSelfDegradation.SetObjectiveDisplayed(10, false)
_SLS_BeggingSelfDegradation.SetObjectiveDisplayed(20, false)

_SLS_BeggingSelfDegradation.SetStage(30)
_SLS_BeggingSelfDegradation.Stop()
_SLS_BeggingSelfDegradation.Start()
Dnpc.ForceRefTo(NewTarget)
_SLS_BeggingSelfDegradation.SetStage(10)
_SLS_BeggingSelfDegradation.SetObjectiveDisplayed(10)
QuestGiver.ForceRefTo(akSpeaker)
Client.ForceRefTo(akSpeaker)
;END CODE
EndFunction
;END FRAGMENT

;END FRAGMENT CODE - Do not edit anything between this and the begin comment

Quest Property _SLS_BeggingSelfDegradation Auto
ReferenceAlias Property QuestGiver Auto
ReferenceAlias Property Client Auto
ReferenceAlias Property Dnpc Auto
